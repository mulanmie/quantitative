...
path = '/data/dataDisk1/mulan/final_data1'
...
class conv1D_lstm_model:
    ...
    def establish(self, day_dim, tick_len, x_dim):
        ...
        shared_conv_layer1 = Conv1D(64, 1, activation='relu', kernel_initializer='random_normal')
        shared_conv_layer2 = Conv1D(64, 1, activation='relu', kernel_initializer='random_normal')
        # shared_maxpool_layer = MaxPooling1D(3)
        shared_conv_layer3 = Conv1D(32, 1, activation='relu', kernel_initializer='random_normal')
        shared_conv_layer4 = Conv1D(32, 1, activation='relu', kernel_initializer='random_normal')
        ...
        for i in range(mem_data.shape[3] - time_step + 1 - 2):
            for j in range(time_step):
                ...
                Y[i0*(mem_data.shape[3] - time_step + 1 - 2)+i, j, 0] = label_data[int(np.argwhere(codes_index1 == codes_share[code]))][i+j+2] if not\
                    np.isnan(label_data[int(np.argwhere(codes_index1 == codes_share[code]))][i+j+2]) else 0
    np.save('/data/dataDisk1/mulan/x_4_3', X)
    np.save('/data/dataDisk1/mulan/y_4_3', Y)
...
if __name__ == '__main__':
    ...
    test_x = list(np.load('/data/dataDisk1/mulan/x_3_1.npy'))
    test_y = np.load('/data/dataDisk1/mulan/y_3_1.npy')[:, 4, 0]
